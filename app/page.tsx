              ...
              <div className="flex flex-col h-full">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <span className="inline-block px-3 py-1 rounded-full text-xs font-mono bg-white/10 text-white/70 border border-white/5">
                      {PROFILE.role}
                    </span>
                  </div>
                  <div className="hidden md:flex items-center gap-2">
                    <span className="text-[10px] font-bold text-white/50 tracking-widest">SPEAKING</span>
                    <div className="flex gap-1 items-end h-4">
                      {[...Array(5)].map((_, i) => (
                        <motion.div
                          key={i}
                          className={`w-0.5 rounded-full ${i < 2 || i > 3 ? 'bg-white/30' : 'bg-white'}`}
                          animate={{
                            height: [4, 12, 6, 14, 4],
                          }}
                          transition={{
